...
_original = PrintixClient.search_card


def install():
    def wrapper(self, card_id=None, card_number=None):
        if card_id:
            return _original(self, card_id=card_id, card_number=None)
        if not card_number:
        ...
        for candidate in _candidates(card_number):
            ...
            try:
                result = _original(self, card_id=None, card_number=candidate)
    ...
    PrintixClient.search_card = wrapper
