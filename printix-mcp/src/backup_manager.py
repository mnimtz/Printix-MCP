...
# v6.9.3 (Addon-Variante): HA Supervisor mountet `/backup` via
# `map: backup:rw` in config.yaml — Backups landen damit im HA-
# Standard-Sicherungsvolume und werden von HA-Snapshots
# automatisch mit-erfasst.
# Fallback: falls /backup aus irgendeinem Grund nicht beschreibbar
# ist (z.B. älteres HA-Setup ohne backup-Map), nutzen wir
# /data/backups innerhalb des persistenten Add-on-Volumes.
def _resolve_default_backup_dir() -> str:
    primary = "/backup/printix-mcp"
    try:
        ...
        pass
    return str(DATA_DIR / "backups")

BACKUP_DIR = Path(os.environ.get("BACKUP_DIR", _resolve_default_backup_dir()))
